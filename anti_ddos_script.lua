...
--Made our Javascript puzzle a little bit more dynamic than the static equation above it will change every 24 hours
...
--[[
Encrypt/Obfuscate Javascript output to prevent content scrappers and bots decrypting it to try and bypass the browser auth checks
...
Traffic to your site from facebook Posts / Shares the URL's will all contain this .com/index.html?fbclid=xyz
]]
...
local function WAF_URI_Request()
	if next(WAF_URI_Request_table) ~= nil then --Check Post filter table has rules inside it
		...
		--[[So to keep the security strong I match the same version your web application would need protecting from 
		(Yes the encoded copy that could contain malicious / exploitable contents)]]
...
local function encrypt_javascript(string1, type, defer_async, num_encrypt, encrypt_type, methods) --Function to generate encrypted/obfuscated output
	...
	if type == 5 then --Javascript Scrambler (Obfuscate Javascript by putting it into vars and shuffling them like a deck of cards)
...
<a href="https://github.com/SarthakSardesai1/anti-ddos-using-nginx-lua" target="_blank">DDoS protection by &copy; Sarthak Sardesai & Kaiwalya Patil</a>
</div>
]]
...
ngx.header["Expires"] = "0"
if credits == 1 then
ngx.header["Anti-DDoS"] = "https://github.com/SarthakSardesai1/anti-ddos-using-nginx-lua"
end
ngx.header.content_type = "text/html; charset=" .. default_charset
